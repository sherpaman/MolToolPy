...
class TimeSer(np.ndarray):
        def __init__(self,data,n_data,dim,nbins=12,bins=None,prob=None,do_reshape=True,frame_row=True,dtype=float):
                ...
                #self.bins   = [ np.linspace(np.min(self),np.max(self),nbins+1) ] # IT MUST BE A *LIST* OF *1-DIM ARRAYS*
                self.bins   = bins
                self.do_reshape= reshape
                self.prod=np.ones(dim+1,dtype=int)
                self.frame_row=frame_row
                self._shape(force=self.do_reshape)
                self.prob=prob
                if self.prob == None:
                        self.prob_av=False
                else:
                        ...
                        if self.frame_row:
                                ...
                                for i in np.arange(self.rep):
                                        for j in np.arange(self.dim):
                                                for k in np.arange(self.n_data):
                                                        ...
                                                        proper_shape_data [i,j,k] = self[ k , i * self.dim + j ]
                        else:
                                ...
                                for i in np.arange(self.rep):
                                        for j in np.arange(self.dim):
                                                for k in np.arange(self.n_data):
                                                        # OUT_DATA [ N_rep, DIM, N_sample ]
                                                        proper_shape_data [i,j,k] = self[ i * self.dim + j, k ]
                self = proper_shape_data
                return

        def _check_shape(self):
                return ( self.shape == ( self.rep, self.dim, self.n_data ) )
        ...
        def calc_bins(self):
                ...
                for d in np.arange(self.dim):
                        self.bins.append(np.linspace(np.min(self[:,d,:]),np.max(self[:,d,:]),self.nbins+1))
                return
                                
        def calc_prob(self):
                if self.do_reshape == True:
                        self._proper_shape()
        ...
        def mutual_info_bootstrap(self,resample=100,joint=False):
                ...
                if joint:
                        for n in np.arange(resample):
                                ...
                                sample = TimeSer(self[...,c],self.n_data,self.dim,self.nbins,reshape=False,dtype=self.dtype)
                        ...
                        for n in np.arange(resample):
                                ...
                                for r in np.arange(self.rep):
                                        c = np.random.choice(np.arange(self.n_data),self.n_data,replace=True)
                                        d.append(self[r,:,c].reshape((self.dim,self.n_data)))
        ...
        def mutual_info(self):
                ...
                for s in np.arange(self.rep - 1):
                        ...
                        for o in np.arange(s+1,self.rep):
                                ...
                                DATA = np.transpose(np.vstack((self[s],self[o])))
        ...
        def mutual_info_for(self):
                ...
                M       = np.zeros((self.rep,self.rep))
                if self.dtype == float :
                        M, E_joint, P_joint = mi.r_mutualinfo(np.transpose(self),self.entropy,self.bins,self.n_data,self.rep,self.nbins)
                elif self.dtype == int :
                        M, E_joint, P_joint = mi.i_mutualinfo(np.transpose(self),self.entropy,self.bins,self.n_data,self.rep,self.nbins)
        ...
        def mutual_info_simple(self,nrep1=0):
                ...
                for s in np.arange(self.rep):
                        if s != nrep1:
                                DATA = np.transpose(np.vstack((self[nrep1],self[s])))
                                histo , null = np.histogramdd(DATA,bins=(self.nbins,self.nbins)) # concatenate bins list with "+"
        ...
        def mutual_info_simple_for(self,nrep1=0):
                ...
                if self.dtype == float :
                        M, E_joint, P_joint = mi.r_mutualinfo_simp\
                                (np.transpose(self),self.entropy,self.bins,nrep1,self.n_data,self.rep,self.nbins) 
                elif self.dtype == int :
        ...
        def mutual_info_other(self,other):
                ...
                for s in np.arange(self.rep):
                        for o in np.arange(other.rep):
                                ...
                                DATA = np.transpose(np.vstack((self[s],other.data[o])))
        ...
        def digitized(self,replicas=None):
                ...
                other  = TimeSer(np.zeros([replicas,self.dim,self.n_data],dtype=int),self.n_data,self.dim,self.nbins)
                ...
                for r in replicas:
                        for d in np.arange(self.dim):
                                for k in np.arange(self.n_data):
                                        ...
                                        other[r,d,k] = int(np.max(np.where( self.bins[d][:-1] <= self[r,d,k] )))
                #else:
                        #for r in np.arange(self.rep):
                                #for d in np.arange(self.dim):
                                        #for k in np.arange(self.n_data):
