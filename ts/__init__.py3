...
class TimeSer:
    ...
    def calc_prob_weight(self,w):
        ...
        for i in np.arange(self.rep):
            ...
            if np.any(self.bins == None):
                self.calc_bins()
            if self.dim == 1:
                histo , null = np.histogram(DATA,weights=w,bins=self.bins[0])
                self.prob[i] = 1.0 * histo / float(self.n_data)
            else:
                histo , null = np.histogramdd(np.transpose(DATA),weights=w,bins=self.bins)
                self.prob[i] = 1.0 * histo / float(self.n_data)
        self.prob_av=True
        return
    ...
    def calc_entropy_weight(self,w,force=False):
        '''
    ...
    def calc_entropy_traj_omp_weight(self,w,force=False):
        '''
        ...
        if ( self.entropy_av & (not force) ):
            ...
            return
        self.entropy = np.zeros(self.rep)
        self.entropy = mi_omp.entropy_traj_weight(np.transpose(self.data),w,self.n_data,self.rep)
        self.entropy_av = True
        return
    ...
    def mutual_info_traj_omp_weight(self,w):
        ...
        E_joint = np.zeros((self.rep,self.rep))
        M       = np.zeros((self.rep,self.rep))            
        M, E_joint = mi_omp.mutualinfo_traj_weight(np.transpose(self.data),self.entropy)
        return np.transpose(M), np.transpose(E_joint)
    ...
    def mutual_info_omp_weight(self,w):
        ...
        if self.dim > 1:
            ...
            M, E_joint = mi_omp.i_mutualinfo_weight(np.transpose(s.data),s.entropy,w,s.bins,s.n_data,s.rep)
        else:
            self.calc_entropy_weight(w)
            if self.dtype == float :
                M, E_joint = mi_omp.r_mutualinfo_weight(np.transpose(self.data),self.entropy,w,self.bins,self.n_data,self.rep)
            elif self.dtype == int :
                M, E_joint = mi_omp.i_mutualinfo_weight(np.transpose(self.data),self.entropy,w,self.bins,self.n_data,self.rep)
        return np.transpose(M), np.transpose(E_joint)
    ...
    def mutual_info_other_traj_omp_weight(self,other,w):
        ...
        d1      = np.transpose(self.data)
        d2      = np.transpose(other.data)
        M, E_joint =  mi_omp.mutualinfo_other_traj_weight(d1,d2, \
            self.entropy, other.entropy,w)
        return np.transpose(M), np.transpose(E_joint)
    ...
    def mutual_info_other_omp_weight(self,other,w):
        ...
        if (s.dtype == int):
            if (o.dtype == int):
                M, E_joint =  mi_omp.i_mutualinfo_other_weight(d1,d2, \
                    s.entropy, o.entropy, \
                    w,s.bins, o.bins)
            elif (other.dtype == float):
                M, E_joint = mi_omp.ir_mutualinfo_other_weight(d1,d2, \
                    s.entropy, o.entropy, \
                    w,s.bins, o.bins)
        elif (self.dtype == float):
            if (other.dtype == int):
                M, E_joint = mi_omp.ri_mutualinfo_other_weight(d1,d2, \
                    s.entropy, o.entropy, \
                    w, s.bins,    o.bins)
            elif (other.dtype == float):
                M, E_joint =  mi_omp.r_mutualinfo_other_weight(d1,d2, \
                    s.entropy, o.entropy, \
                    w, s.bins, o.bins)
        return np.transpose(M), np.transpose(E_joint)
        ...
        if (self.dtype == int):
            if (other.dtype == int):
                M, E_joint, P_joint =  mi.i_mutualinfo_other_prob(d1,d2, \
                    ...
                    self.n_data, self.rep,other.rep)
            elif (other.dtype == float):
