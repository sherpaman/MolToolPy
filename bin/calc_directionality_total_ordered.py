...
# VAR ARGUMENTS
#
parser.add_argument("-t","--time",dest="t",action="store",type=int,default=2,   help="time frames used to calculate transfer", metavar="INTEGER")
...
#residues = np.arange(res_a,res_b+1)
D = np.zeros((res_b-res_a+1,skip))
f=open(f_out,'w')
for j in np.arange(res_b-res_a+1):
		...
		for i in range(0,skip):
			...
			D[j,i] = CHI_skip[i].direction_other_for(SS,time=t)
			f.write("{0:8.3f} ".format(D[j,i]))
		f.write("{0:8.3f} \n".format(np.mean(D[j,:])))
