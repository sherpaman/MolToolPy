...
parser.add_argument("-m","--mat",dest="file_mut",action="store",type=str,default=None,help="Mutual Info File",required=True,metavar="DAT FILE")
#
# OUTPUT FILES
#
...
parser.add_argument("--col1",dest="col1",action="store",type=int,default=1,   help="first column to read",            metavar="INTEGER")
parser.add_argument("--col2",dest="col2",action="store",type=int,default=-1,help="last column to read",             metavar="INTEGER")
parser.add_argument("--fr1", dest="fr1", action="store",type=int,default=1,   help="first line to read",              metavar="INTEGER")
...
hbonds  = hb.hbonds(mol=mol,file_xpm=options.file_xpm,log=options.file_log)
...
DATA = DATA[options.col1:options.col2,options.fr1:options.fr2:options.skip]

nframes, ncol = DATA.shape

hb_ts = ts(data=DATA,n_data=nframes,dim=1,nbins=2,bins=[np.array([0.0, 0.5, 1.0])])
